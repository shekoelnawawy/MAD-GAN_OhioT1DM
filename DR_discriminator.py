...
def discriminator_o(x, hidden_units_d, reuse=False, parameters=None):
    ...
    with tf.compat.v1.variable_scope("discriminator_0") as scope:
        ...
        cell = tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=hidden_units_d, state_is_tuple=True, reuse=reuse)
...
def invert(settings, samples, para_path, g_tolerance=None, e_tolerance=0.1,
    ...
    # create VARIABLE Z
    Z = tf.compat.v1.get_variable(name='Z', shape=[1, settings['seq_length'],
                                         settings['latent_dim']],
    ...
    similarity_per_sample = tf.linalg.tensor_diag_part(Kxy)
