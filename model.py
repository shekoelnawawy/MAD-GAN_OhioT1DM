...
def GAN_solvers(D_loss, G_loss, learning_rate, batch_size, total_examples, l2norm_bound, batches_per_lot, sigma, dp=False):
    ...
    if dp:
        ...
        D_solver = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(D_loss_mean_over_batch, var_list=discriminator_vars)
...
def generator(z, hidden_units_g, seq_length, batch_size, num_signals, reuse=False, parameters=None, learn_scale=True):
    ...
    with tf.compat.v1.variable_scope("generator") as scope:
        ...
        W_out_G = tf.compat.v1.get_variable(name='W_out_G', shape=[hidden_units_g, num_signals],
        ...
        scale_out_G = tf.compat.v1.get_variable(name='scale_out_G', shape=1, initializer=scale_out_G_initializer,
...
def discriminator(x, hidden_units_d, seq_length, batch_size, reuse=False, parameters=None, batch_mean=False):
    with tf.compat.v1.variable_scope("discriminator") as scope:
        if reuse:
            scope.reuse_variables()
        if parameters is None:
            W_out_D = tf.compat.v1.get_variable(name='W_out_D', shape=[hidden_units_d, 1],
                                      initializer=tf.compat.v1.truncated_normal_initializer())
            b_out_D = tf.compat.v1.get_variable(name='b_out_D', shape=1,
                                      initializer=tf.compat.v1.truncated_normal_initializer())
